...
stop_conditions:
  ...
  - "User asks about SQL Server engine internals or DBA operations — escalate to sql-server-specialist / sql-server-dba"
escalation_rules:
  - trigger: "SQL Server-specific engine internals (execution plans, indexing internals, Query Store) or T-SQL routine design"
    target: "sql-server-specialist"
    reason: "SQL Server has engine-specific internals this agent's cross-dialect scope doesn't cover"
  - trigger: "SQL Server DBA operations (backup/restore, Always On, security, maintenance)"
    target: "sql-server-dba"
    reason: "Operational DBA concerns are outside cross-dialect query optimization"
